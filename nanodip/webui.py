...
class VarsOnDisk:
    """Container that stores variables on disk and guarantees concurrency."""

    def __init__(self, dirname, default={}):
    ...
    def update(self, item, variables):
        """Adds a dictionary of variables to the corresponding item. The
        existing values associated with the item will be replaced by the new
        values provided.
        """
        self.semaphore.acquire()
        combined_vars = self.get(item)
        ...
        if not os.path.exists(itemdir):
            os.makedirs(itemdir)
        with open(itemvars, "w") as f:
            json.dump(combined_vars, f, indent=2)
        self.semaphore.release()

    def get(self, item):
        """Returns all variables associated with the corresponding item.
        Invalid variable files will be removed.
        """
        itemdir = os.path.join(self.dir, item)
        itemvars = os.path.join(itemdir, self.VARSFILE)
...
class UI:
    ...
    devices = VarsOnDisk("devices", default={"termination_type": "manually"})
    samples_stats = VarsOnDisk("samples")
    ...
    def status(self):
        ...
        return render_template(
            ...
            url_all_cpgs=url_for(UI.all_cpgs),
        )
    ...
    def status_samples(self, sample_id="", start_calling="true"):
        ...
        if sample_id:
            UI.samples_stats.update(
                sample_id,
                {"start_calling": start_calling == "true"},
            )
            return start_calling
        analysis_runs = [
            run
            for run in get_runs()
            if not any(
        ...
        status = {}
        progress = {}
        for smp in analysis_runs:
            curr_stats = {
                "time": "",
                "num_completed": 0,
                "num_fastq": 0,
                "no_callable_left": False,
                "called": [""],
                "start_calling": False,
            }
            old_stats = UI.samples_stats.get(smp)
            curr_stats.update(old_stats)
            start_calling[smp] = curr_stats["start_calling"]
            status[smp] = curr_stats["time"]
            if curr_stats["no_callable_left"]:
                status[smp] += " No more callable reads found"
            elif len(curr_stats["called"]) > 0:
                status[smp] += " Methylation called for " + "/".join(
                    curr_stats["called"]
                )
            progress[
                smp
    ...
    def start(
        ...
        if start_now:
            ...
            UI.devices.update(device_id, {"termination_type": "manually"})
            run_ids = start_run(
                device_id=device_id,
                sample_id=sample_id,
    ...
    def set_auto_terminate(self, device_id="", termination_type=""):
        ...
        if termination_type in ["terminated", "manually", "auto"]:
            UI.devices.update(device_id, {"termination_type": termination_type})
            logger.info(
                "Auto terminate status of %s set to %s",
                device_id,
                UI.devices.get(device_id)["termination_type"],
            )
        else:
    ...
    def launch_auto_terminator(device_id=""):
        ...
        if (
            ...
            number_of_called_bases(device_id) > NEEDED_NUMBER_OF_BASES
        ):
            stop_run(device_id)
    ...
    def cpgs(self, sample_name="", max_calls="1"):
        ...
        # If there is exists already a running process, do nothing.
        ...
        if max_calls == "0":
            keep_vars = {
                key: old_stats[key]
                for key in ("time", "called") if key in old_stats
            }
            UI.samples_stats.update(sample_name, keep_vars)
        UI.sem.release("cpg")
    ...
    @cherrypy.expose
    def all_cpgs(self):
        """Starts methylation calling for all selected samples."""
        analysis_runs = [
            run for run in get_runs() if not any(
                pattern in run for pattern in ANALYSIS_EXCLUSION_PATTERNS
            )
