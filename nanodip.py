...
class UserInterface(object):
    ...
    @cherrypy.expose
    def cnv(self, samp, genes=""):
        try:
            cnv_plt_data = CNVData.from_name(samp)
        ...
        def make_plot(cnv_data, lock):
            ...
            lock.acquire()
            if not cnv_data.read_from_disk():
    ...
    def umap(self, samp, ref, close_up=""):
        try:
            umap_data = UMAPData.from_name(samp, ref)
        except FileNotFoundError:
            raise cherrypy.HTTPError(405, "URL not allowed")
        ...
        def make_plot(plt_data, lock):
            """Plot function for multiprocessing."""
            lock.acquire()
            if not plt_data.read_from_disk():
                plt_data.make_umap_plot()
                print("********* samp=", samp,"ref=", ref,"cu=", close_up)
            lock.release()
        ...
        proc.start()
        print("******************START", samp,"ref=", ref,"cu=", close_up)
        proc.join()
        print("******************END", samp,"ref=", ref,"cu=", close_up)
